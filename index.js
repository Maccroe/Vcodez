...
client.on('presenceUpdate', async(oldPresence, newPresence) => 
{
   ...
   var botdata = await botsdata.findOne({ botID: newPresence.userID });
    ...
    if(newPresence.guild.id == "792809995200102410")
    {
...
if(!uptimerate)
        ...
        if(breh === newPresence.userID)
      {
       ...
       client.channels.cache.get("886500316089057350").send(new Discord.MessageEmbed()
       .setThumbnail(bota.displayAvatarURL()).setTitle(`Bot Offline`).setDescription(`<@${newPresence.userID}> is Offline And Uptime Rate - ${uptimerate}%`)) 
   ...
   if(!uptimerate)
      {
       ...
       client.channels.cache.get("886500316089057350").send(new Discord.MessageEmbed()
       .setThumbnail(bota.displayAvatarURL()).setTitle(`Bot Online`).setDescription(`<@${newPresence.userID}> is Online And Uptime Rate - ${uptimerate}% And was Offline for ${hour}h ${minutes}m ${seconds}s`)) 
       db.set(`timefr_${newPresence.userID}`, Date.now())
    }
    }
...
client.on('voiceStateUpdate', async(oldState, newState) => {
  ...
  if (!oldState.channel) {
    ...
    client.channels.cache.get('886500605923823637').send(new Discord.MessageEmbed().setTitle(`Vcodez Coins`).setDescription(`Hey <@${newState.member.id}>, Thankyou for being active! You have got some **Vcodez Coins** for being active!`))
    }
  }
})
